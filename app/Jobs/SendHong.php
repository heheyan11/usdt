    ...
    public function send(){
        ...
        foreach ($plans as $value) {
            ...
            if ($this->plan->manage_rate > 0) {
                ...
                LogForm::create(['message' => '用户' . $value->user->phone . '手续费', 'amount' => $shouxu]);
            }
            //给自己
            $self = bmul($baseMoney, $base_rate);
            $value->user->wallet()->increment('amount', $self);
            $value->user->logincomes()->create(['title' => $this->plan->title, 'income' => $self, 'amount' => $value->amount]);
            //给上线
            $up_1 = bmul($baseMoney, $one_rate);
            if ($value->user->parent_id && $up_1) {
                $parent = $value->user->parent;
                $parent->wallet()->increment('amount', $up_1);
                $parent->logincomes()->create(['title' => '直推好友' . str_phone($value->user->phone) . '提供', 'income' => $up_1, 'amount' => $value->amount]);
                $up_2 = bmul($baseMoney, $two_rate);
                //2级上线
                if ($parent->parent_id && $up_2) {
                    $parent->parent->wallet()->increment('amount', $up_2);
                    $parent->parent->logincomes()->create(['title' => '隔代好友' . str_phone($value->user->phone) . '提供', 'income' => $up_2, 'amount' => $value->amount]);
                } elseif ($up_2) {
                    $plantIncome = badd($plantIncome, $up_2);
                    LogForm::create(['message' => '用户' . $value->user->phone . '没有2级上线', 'amount' => $up_2]);
                }
            } else if ($up_1) {
                $plantIncome = badd($plantIncome, $up_1);
                LogForm::create(['message' => '用户' . $value->user->phone . '没有1级上线', 'amount' => $up_1]);
            }
            ...
            foreach ($up as $baba) {
                if ($baba->share_level == 0 || $baba->share_level < $level) {
                    ...
                    if ($money) {
                        ...
                        LogForm::create(['message' => "{$value->user->phone}的上线{$baba->phone}不满足拿贡献奖条件", 'amount' => $money]);
                    }
                    break;
                    ...
                    $baba->logincomes()->create(['title' => '平级贡献奖 由：' . str_phone($value->user->phone) . '提供.', 'income' => $money, 'amount' => $value->amount]);
                    $plantIncome = bsub($plantIncome, $money);
                    LogForm::create(['message' => "{$value->user->phone}和{$baba->phone}平级，平台支出", 'amount' => -$money]);
                } elseif ($baba->share_level > $level) {
                    ...
                    $baba->wallet()->increment('amount', $money);
                    $baba->logincomes()->create(['title' => '贡献奖 由：' . str_phone($value->user->phone) . '提供', 'income' => $money, 'amount' => $value->amount]);
                }
            ...
            if ($money) {
                $plantIncome = badd($plantIncome, $money);
                LogForm::create(['message' => "剩余贡献奖", 'amount' => $money]);
            }
        }

        $this->plan->logcrows()->create([
